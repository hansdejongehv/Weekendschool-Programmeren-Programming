...
void setup(){    // Dit is openingsaccolade die bij setup hoort.
  ...
  pinMode(LEDGroen, OUTPUT);            // Idem voor de rode LED.
  pinMode(LEDBlauw, OUTPUT);            // Idem voor de rode LED.
...
//  delay(500);  
  ...
  while (true) {                                    // Herhaal dit eindeloos
    if (digitalRead(KnopKlein) == LOW) {            // Als het kleine knopje ingedrukt wordt 
      ...
      for (i=1; i<1000; i++) {                          // doe het 1000 keer om de 
                                                        // een beetje tijd te geven om de 
                                                        // speeltijd in te stellen
        ...
        if (digitalRead(KnopKlein) == LOW) {            // Als het kleine knopje ingedrukt is 
          ...
          while (digitalRead(KnopKlein) == LOW) {  
            PotMeterStand = analogRead(PotMeter);       // lees de nieuwe stand van de potmeter
            LEDWaardeNieuw = LEDWaardeBegin 
              + map(PotMeterStand-PotMeterStandBegin,-1024, 1024, 260, -260);
                                                        // verander de waarde van de LED.
                                                        // Zie de Bijlage C in de handleiding voor 
                                                        // de uitleg
            LEDWaardeNieuw = constrain(LEDWaardeNieuw, 0, 255);
                                                        // Begrens de waarde tussen 0 en 255
            analogWrite(LEDRood, LEDWaardeNieuw);       // Zet de nieuwe waarde voor de LED
